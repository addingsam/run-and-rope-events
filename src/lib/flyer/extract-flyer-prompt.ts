...
- disciplines must use only allowed labels. Include every distinct discipline clearly listed on the flyer. On rodeo flyers, include rough stock, timed events, and youth events when shown (for example Bareback, Barrel Racing, Pole Bending, Team Roping).
- Map discipline abbreviations and organization names to the allowed labels when confident. Examples: CMSA or Cowboy Mounted Shooting Association -> "Cowboy Mounted Shooting"; BB or Bareback -> "Bareback Riding (BB)"; SB or Saddle Bronc -> "Saddle Bronc (SB)"; BR or Bull Riding -> "Bull Riding (BR)"; RB or Ranch Bronc -> "Ranch Bronc Riding (RB)"; CBR or CGBR -> "Barrel Racing (CBR/CGBR)"; Poles or Pole Bending -> "Pole Bending (Poles)"; SW, BD, or Bull Dogging -> "Steer Wrestling / Bull Dogging (SW/BD)"; BA, CBA, BAW, or CGBKR -> "Breakaway Roping (BA/CBA/BAW/CGBKR)"; SR or SRADM -> "Steer Roping (SR/SRADM)"; CR, TD, Tie Down, or Tie Down Roping -> "Calf Roping / Tie Down Roping (CR/TD)".
- format must be exactly "Jackpot" or "Rodeo" or null. When the flyer states it is a rodeo at any level (Open, Youth, Amateur, Ranch, Pro, or the word "rodeo" in the event title), set format to "Rodeo" regardless of which individual events or disciplines are listed. Rodeo format trumps jackpot categorization.
- Code Pole Bending the same as Barrel Racing: either may be a Jackpot or Rodeo event. When the flyer is a rodeo at any level, use format "Rodeo" even if the only listed event is poles or barrels. Use format "Jackpot" for standalone barrel or pole events only when the flyer is not a rodeo.
- Cowboy Mounted Shooting, Ranch Horse, and Obstacle & Trail are jackpot-only when the flyer is not a rodeo. When those appear on a rodeo flyer, keep format "Rodeo".
...
- NextGen Rodeo single-day performances with multiple disciplines at one location (for example BB, BR, SB, RB, and CBR on the same date) are Jackpot events — set format to "Jackpot" even when the flyer mentions IPRA, NCPRA, or other rodeo associations, unless the flyer explicitly states it is a rodeo at any level.
...
- producerWebsite: extract the event producer, stock contractor, or organization's website when printed on the flyer. Include values such as www.example.com, example.com, or full https:// URLs. When the flyer shows NextGen Rodeo entry instructions, QR codes, or URLs such as app.nextgenrodeo.com (with or without https://), put that URL in producerWebsite. When the flyer references SGP, SGP Events, sgpevents.com, or sgpevents.net, use www.sgpevents.net. When the flyer references Saddle Book, Saddlebook, or saddlebook.com, use https://saddlebook.com. Do not include social media profile URLs unless that is the only website shown.
