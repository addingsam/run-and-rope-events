...
- producerWebsite: extract the event producer, stock contractor, or organization's website when printed on the flyer. Include values such as www.example.com, example.com, or full https:// URLs. When the flyer shows NextGen Rodeo entry instructions, QR codes, or URLs such as app.nextgenrodeo.com (with or without https://), put that URL in producerWebsite. When the flyer references SGP, SGP Events, sgpevents.com, or sgpevents.net, use www.sgpevents.net. Do not include social media profile URLs unless that is the only website shown.
